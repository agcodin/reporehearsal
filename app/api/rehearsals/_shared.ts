...
export async function sessionRequest(request:Request):Promise<{user:SessionUser;token:string|null}>{await consumeRateLimit(request,"rehearsals",120,60);const user=await getAuthenticatedUser();return{user:user?{email:user.email,displayName:user.displayName}:null,token:request.headers.get("x-rehearsal-access")}}
