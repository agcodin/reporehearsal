...
const schema=z.object({repositoryId:z.string().min(3).max(100),repositoryAccessToken:z.string().min(20).max(200).nullable().optional(),incidentTemplateId:z.string().min(3).max(100),incidentCandidateId:z.string().min(3).max(80).optional(),difficulty:z.enum(["BEGINNER","INTERMEDIATE","ADVANCED"]),mode:z.enum(["GUIDED","INDEPENDENT","INTERVIEW"]),timeLimitMinutes:z.number().int().min(15).max(60)});
