...
export async function POST(request: Request) {
  ...
  try {
    ...
    const stored = await saveRepository({ email: user.email, displayName: user.displayName }, { source: "GITHUB_CONNECTED", externalRef: repository.id, name: repository.name, analysis: { ...analysis, sourceUrl: repository.sourceUrl, fullName: repository.fullName } as unknown as Record<string, unknown>, files });
    await saveAccountRepository(user.email, user.displayName, { id: stored.repository.id, source: "GITHUB_CONNECTED", externalId: repository.id, name: repository.name, displayRef: repository.fullName, ...stack, fileCount: repository.fileCount });
    return NextResponse.json({ repository: { ...repository, id: stored.repository.id, stack, compatibleIncidentIds, detectedFiles, warnings: [...repository.warnings, ...(compatibleIncidentIds.length ? [] : ["No safe, repairable source boundary was found for automatic incident generation."])] }, accessToken: stored.accessToken, savedToAccount: true, sourceModified: false, correlationId });
  } catch (error) {
