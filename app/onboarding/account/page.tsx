...
import { planFor, priceForCadence } from "../../../src/billing/plans";
...
export default async function AccountStep({ searchParams }: { searchParams: Promise<OnboardingQuery> }) {
  ...
  const next = onboardingPath("plan", selection.plan, selection.billing);
  const user = await getAuthenticatedUser();
  const selectedPlan = selection.plan === "FREE" ? null : { plan: planFor(selection.plan), price: priceForCadence(planFor(selection.plan), selection.billing) };
  return <main className="onboarding-page"><div className="onboarding-shell">
    ...
    <section className="onboarding-panel onboarding-account">
      ...
      <p>{user ? "Confirm the account below, then choose your access." : "An account keeps your repository library, practice history, scores, and plan settings together."}</p>
      {selectedPlan && <p className="plan-intent">Selected: <b>{selectedPlan.plan.name}</b> · {selectedPlan.price.price} {selectedPlan.price.cadence}</p>}
      ...
      <div className="onboarding-actions">{user ? <Link className="button button-dark" href={next}>Continue to plans →</Link> : <Link className="button button-dark" href={signInPath(next)}>Sign in or create account →</Link>}<Link className="text-link" href="/pricing">Back to pricing</Link></div>
