...
export default function Report({ sessionId }: { sessionId: string }) {
  ...
  async function shareResult() {
    ...
    try {
      ...
      catch { setShareStatus("Copy this page URL to share your result"); }
    }
  }

  async function publishVerifiedResult(){setShareStatus("Publishing verified result…");try{const response=await fetch(`/api/rehearsals/${sessionId}/publish`,{method:"POST",headers:token?{"x-rehearsal-access":token}:{}});const data=await response.json() as{verifyPath?:string;error?:{message?:string}};if(!response.ok||!data.verifyPath)throw new Error(data.error?.message??"The result could not be published.");setVerifiedPath(data.verifyPath);await navigator.clipboard.writeText(new URL(data.verifyPath,window.location.origin).toString());setShareStatus("Verified result published and copied");}catch(cause){setShareStatus(cause instanceof Error?cause.message:"The result could not be published.")}}
