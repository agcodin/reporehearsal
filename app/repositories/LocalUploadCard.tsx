...
export default function LocalUploadCard() {
  ...
  async function upload(files: File[], paths: string[], name: string, archive = false) {
    ...
    try {
      ...
      const result = await response.json() as { repository?: UploadResult; savedToAccount?: boolean; error?: { message?: string } };
      if (!response.ok || !result.repository) throw new Error(result.error?.message ?? "The codebase could not be analyzed.");
      setRepository(result.repository); setMessage(result.savedToAccount ? "Repository metadata was saved to your account." : "Analysis complete. Sign in if you want this repository listed on your dashboard."); setStatus("success");
