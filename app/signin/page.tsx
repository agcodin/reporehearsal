...
export default async function SignInPage() {
  ...
  return <main className="signin-page"><section className="signin-card"><p className="eyebrow">OPTIONAL ACCOUNT</p><h1>Save your rehearsal progress</h1><p>RepoRehearsal is public and the demo, public GitHub import, and local upload all work without an account. Sign in when you want history, preferences, and safe repository snapshots to follow you between sessions.</p><Link className="provider-button" href={chatGPTSignInPath("/dashboard")}><span className="provider-mark">OA</span><span><b>Continue with ChatGPT</b><small>Secure sign-in is handled by OpenAI</small></span><strong>→</strong></Link><div className="signin-divider"><span>ACCOUNT BOUNDARY</span></div><ul className="signin-benefits"><li>Save scores, duration, and completed rehearsals</li><li>Remember guided, independent, or interview defaults</li><li>Reuse uploaded and public GitHub source snapshots</li><li>Provider passwords, excluded secrets, and original repositories are never stored</li></ul><Link className="text-link" href="/repositories">Continue without an account →</Link></section></main>;
}
