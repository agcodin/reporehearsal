...
RepoRehearsal is a Next.js App Router application deployed through the Sites-compatible Vinext runtime. Public visitors can use the demo, public GitHub import, and local folder/ZIP analysis without an account. Dispatch-owned ChatGPT sign-in optionally identifies users, and D1 stores profiles, preferences, user-owned rehearsal summaries, and repository analysis metadata. The hackathon build uses a deterministic in-process demonstration adapter because Docker and PostgreSQL are unavailable in this environment. The adapter implements the same boundaries as the planned Docker adapter: immutable source, per-session copied workspaces, approved command IDs, path containment, expiration, evidence recording, deterministic fault injection, and hidden validation.
...
flowchart LR
  ...
  Intake[GitHub URL · folder · ZIP] --> Analyzer
  Auth[Optional ChatGPT sign-in] --> Sessions
  Sessions --> D1[Profiles + rehearsal/repository metadata]
```
