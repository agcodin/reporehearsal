...
import type { AccountDashboard, AccountMode, AccountProfile, AccountRehearsal } from "./types";
...
export async function getAccountDashboard(email: string, displayName: string): Promise<AccountDashboard> {
  ...
  return { profile, rehearsals, metrics: calculateAccountMetrics(rehearsals) };
}
