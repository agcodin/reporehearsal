...
export function evaluateGeneratedIncident(blueprint: GeneratedIncidentBlueprint, files: WorkspaceFile[], context: EvaluationContext): ValidationResult {
  ...
  const breakdown = [
    ...
    { label: "Investigation", earned: investigation, possible: 20, signals: [`${plural(openedFiles.size, "file")} opened`, `${plural(commands.size, "distinct command")} or evidence source used`, firstEdit < 0 ? "No file was edited" : `${plural(evidenceBeforeEdit, "investigation step")} before the first edit`] },
    { label: "Fix quality", earned: fixQuality, possible: 25, signals: [repaired ? "Incident behavior restored" : "Original symptom still reproducible", unsafe ? "Unsafe shortcut detected" : "No unsafe shortcut detected", editedPaths.length ? `${plural(editedPaths.length, "file")} edited` : "No file was edited"] },
    { label: "Verification", earned: verification, possible: 15, signals: [ranCommands.length ? `Ran ${ranCommands.join(", ")}` : "No verification command was run", commands.has("run-tests") ? "Tests were run" : "Tests were never run"] },
