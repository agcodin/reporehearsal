...
async function ownerTeam(user: AuthenticatedUser): Promise<TeamRow> {
  ...
  await db.prepare("INSERT INTO teams (id, owner_account_id, name, created_at, updated_at) VALUES (?, ?, ?, ?, ?) ON CONFLICT(owner_account_id) DO NOTHING")
    .bind(crypto.randomUUID(), profile.id, `${profile.displayName}'s team`, now, now).run();
  const team = await db.prepare("SELECT id, name, owner_account_id, created_at FROM teams WHERE owner_account_id = ?").bind(profile.id).first<TeamRow>();
  ...
  await db.prepare(`INSERT INTO team_members (id, team_id, account_id, email, display_name, role, status, invited_at, joined_at)
    ...
    .bind(crypto.randomUUID(), team.id, profile.id, profile.email, profile.displayName, now, now).run();
...
export async function getTeamDashboard(user: AuthenticatedUser) {
  ...
  const assignmentResult = await db.prepare("SELECT id, repository_id, repository_name, incident_template_id, incident_name, assigned_to_email, created_at FROM team_assignments WHERE team_id = ? ORDER BY created_at DESC LIMIT 100").bind(team.id).all<AssignmentRow>();
  const repositoryResult = await db.prepare("SELECT id, name, display_ref, language, file_count FROM account_repositories WHERE account_id = ? ORDER BY updated_at DESC LIMIT 100").bind(team.owner_account_id).all<{ id: string; name: string; display_ref: string; language: string; file_count: number }>();
  const resultRows = await db.prepare(`SELECT r.id, m.email AS member_email, m.display_name, r.incident_name, r.repository_name, r.score, r.duration_minutes, r.status, r.completed_at
    ...
    WHERE m.team_id = ? ORDER BY r.completed_at DESC LIMIT 100`).bind(team.id).all<ResultRow>();
...
export async function getAssignedTeamWork(user: AuthenticatedUser) {
  await ensureTeamSchema();
  const profile = await ensureAccount(user.email, user.displayName); const db = database(); const now = new Date().toISOString();
  await db.prepare("UPDATE team_members SET account_id = ?, display_name = ?, status = 'ACTIVE', joined_at = COALESCE(joined_at, ?) WHERE email = ?")
    .bind(profile.id, profile.displayName, now, profile.email).run();
...
export async function createTeamAssignment(user: AuthenticatedUser, input: { repositoryId: string; incidentTemplateId: string; incidentName: string; assignedToEmail: string }) {
  ...
  if (input.repositoryId === "challenge-of-the-day") repositoryName = "Challenge of the Day";
  ...
  await db.prepare("INSERT INTO team_assignments (id, team_id, repository_id, repository_name, incident_template_id, incident_name, assigned_to_email, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)")
    .bind(crypto.randomUUID(), team.id, input.repositoryId, repositoryName, input.incidentTemplateId, input.incidentName, input.assignedToEmail, now).run();
  return getTeamDashboard(user);
}
